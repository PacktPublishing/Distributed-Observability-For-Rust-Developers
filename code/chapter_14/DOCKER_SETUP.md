...
3. **Loki** - Log aggregation backend receiving logs via OTLP HTTP
4. **Grafana** - Dashboard visualization for metrics, traces, and logs (datasources auto-provisioned)
5. **PostgreSQL Database** - Sets up with schema initialization
6. **Products Service** - Runs database migrations
7. **Inventory Service** - Stock and pricing management
8. **Orders Service** - Order processing and checkout
9. **Fraud Scoring Service** - GenAI-powered fraud detection (mock or Anthropic)
10. **Data Ingestion** - Loads 2000 products into the database
11. **OtelMart Service** - Main e-commerce application (Rust + Angular)
...
loki (started) → receives logs via OTLP HTTP (port 3100)
grafana (started) → queries Prometheus, Jaeger, and Loki for dashboards
    ↓
postgres (healthy) → initializes schemas from init/*.sql
    ↓
fraud_scoring (started) → scores orders for fraud via GenAI (mock by default)
    ↓
products, inventory, orders (started) → connect to database, send traces to Jaeger, push metrics to Prometheus, send logs to Loki
    ↓
data-ingestion (waits 10s) → loads 2000 products + specs
    ↓
otelmart (started) → serves UI and API, sends traces to Jaeger, pushes metrics to Prometheus, sends logs to Loki
```
...
- **OTLP Receiver**: `http://loki:3100/otlp/v1/logs`
- **Config**: `loki-config.yaml`
- **Volume**: `loki_data`
- **Purpose**: Log aggregation via native OTLP ingestion; indexes `service.name` as a label for easy filtering
...
- **Purpose**: Dashboard visualization for metrics (Prometheus), traces (Jaeger), and logs (Loki)
- **Data Sources**: Auto-provisioned via `grafana-datasources.yaml` — Prometheus (`http://prometheus:9090`), Jaeger (`http://jaeger:16686`), Loki (`http://loki:3100`)
- **Log-Trace Correlation**: Loki datasource is configured with a derived field to link `trace_id` in log lines to Jaeger traces
...
### Fraud Scoring Service (Port 3004)
- **Container**: `fraud-scoring-service`
- **API**: http://localhost:3004
- **Endpoints**:
  - `POST /score` - Score an order for fraud risk
  - `GET /health` - Health check
- **Provider**: Mock by default (set `FRAUD_SCORING_PROVIDER_NAME=anthropic` and `ANTHROPIC_API_KEY` for real API)
- **Models**: Two-tier routing — Haiku (low risk) / Opus (high risk)
- **Telemetry**: GenAI semantic conventions (`gen_ai.*` span attributes and metrics)
- See [fraud_scoring/README.md](fraud_scoring/README.md) for full configuration
...
- **Logging**: All Rust services send logs to Loki via OTLP HTTP (`/otlp/v1/logs`)
- **Telemetry Env Vars**: Each service sets `OTEL_EXPORTER_OTLP_TRACES_ENDPOINT`, `OTEL_EXPORTER_OTLP_METRICS_ENDPOINT`, and `OTEL_EXPORTER_OTLP_LOGS_ENDPOINT`
...
2. Data sources (Prometheus, Jaeger, Loki) are **auto-provisioned** — no manual setup needed
3. Use **Explore** to run PromQL queries, view traces, or search logs
