...
//! with automatic configuration of the `inventory` schema search path.
//!
//! # Schema Search Path
//! All connections are configured to use `search_path TO inventory, products, public`,
//! which means queries don't need to prefix table names with `inventory.`
...
impl Database {
    ...
    /// 3. Sets application name to "inventory-service"
    /// 4. Creates a connection pool with the specified max connections
    /// 5. Configures each connection to use the `inventory` schema by default
    ///
    /// # Arguments
    ...
    pub async fn new(database_url: &str, max_connections: u32) -> Result<Self> {
        ...
        connect_opts = connect_opts.application_name("inventory-service");
    ...
    /// ```no_run
    /// let pool = db.pool();
    /// let inventory = sqlx::query!("SELECT * FROM product_inventory").fetch_all(pool).await?;
    /// ```
