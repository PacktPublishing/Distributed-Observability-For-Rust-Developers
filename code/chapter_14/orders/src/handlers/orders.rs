...
/// Request to the fraud scoring service
#[derive(Debug, serde::Serialize)]
struct FraudScoreRequest {
    ...
    has_international_shipping: bool,
    flagged_product_category: bool,
}

/// Response from the fraud scoring service
#[derive(Debug, serde::Deserialize)]
...
/// Call the fraud scoring service to assess order risk.
///
/// Returns the fraud decision. On any error (network, timeout, etc.),
/// defaults to "manual_review" so checkout is not blocked.
#[instrument(
    name = "score_fraud",
    skip(state),
    fields(fraud.decision = tracing::field::Empty)
)]
async fn score_fraud(
    state: &AppState,
    order_total: f64,
    ...
    let url = format!("{}/score", state.fraud_scoring_service_url());

    let is_international = shipping_country != "US" && shipping_country != "USA";

    let body = FraudScoreRequest {
        order_total,
        is_returning_customer: false, // simplified: real impl would check customer history
        product_names: product_names.to_vec(),
        gift_message: gift_message.clone(),
        has_international_shipping: is_international,
    ...
    let decision = match result {
        Ok(resp) if resp.status().is_success() => {
            match resp.json::<FraudScoreResponse>().await {
                Ok(score) => score.decision,
                Err(e) => {
                    tracing::warn!(
                        error.r#type = "deserialization",
                        error.message = %e,
                        "Failed to parse fraud scoring response, defaulting to manual_review"
                    );
        ...
        Ok(resp) => {
            tracing::warn!(
                ...
                "Fraud scoring service returned error, defaulting to manual_review"
            );
            "manual_review".to_string()
        }
        Err(e) => {
            tracing::warn!(
                ...
                error.message = %e,
                "Failed to reach fraud scoring service, defaulting to manual_review"
            );
            "manual_review".to_string()
        }
    };

    tracing::Span::current().record("fraud.decision", decision.as_str());
    decision
}
...
async fn release_all_reserved_stock(state: &AppState, reserved_items: &[(Uuid, i32)]) {
    ...
    tracing::info!(released_count = reserved_items.len(), "Stock release complete");
}
...
pub async fn create_order(
    ...
    tracing::info!(
        ...
        "All stock reserved, scoring for fraud"
    );

    // Score the order for fraud before committing the transaction
    let product_names: Vec<String> = request.items.iter().map(|i| i.product_name.clone()).collect();
    let order_total_f64: f64 = request
        .items
        .iter()
        .map(|i| {
            let price_f64 = i.unit_price.to_string().parse::<f64>().unwrap_or(0.0);
            price_f64 * f64::from(i.quantity)
        })
        .sum();

    let fraud_decision = score_fraud(
        &state,
        order_total_f64,
        &product_names,
    ...
    if fraud_decision == "rejected" {
        tracing::warn!(
            fraud.decision = "rejected",
            "Order rejected by fraud scoring, releasing reserved stock"
        );
        record_checkout_failure("fraud_rejected", &start);
        release_all_reserved_stock(&state, &reserved_items).await;
        return (
            StatusCode::FORBIDDEN,
            Json(serde_json::json!({
                "error": "Order rejected",
                "message": "This order has been flagged and cannot be processed"
            })),
        )
            .into_response();
    }
    ...
    tracing::info!(
        fraud.decision = %fraud_decision,
        "Fraud scoring complete, persisting order"
    );
