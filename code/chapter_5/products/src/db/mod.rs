...
//! with automatic configuration of the `products` schema search path.
//!
//! # Schema Search Path
//! All connections are configured to use `search_path TO products, public`,
//! which means queries don't need to prefix table names with `products.`
//!
//! # Connection Pool
...
impl Database {
    ...
    /// 3. Sets application name to "products-service"
    /// 4. Creates a connection pool with the specified max connections
    /// 5. Configures each connection to use the `products` schema by default
    ///
    /// # Arguments
    ...
    pub async fn new(database_url: &str, max_connections: u32) -> Result<Self> {
        ...
        connect_opts = connect_opts.application_name("products-service");
        ...
        let pool = PgPoolOptions::new()
            ...
            .after_connect(|conn, _meta| {
                Box::pin(async move {
                    ...
                    // Example: SELECT * FROM products instead of SELECT * FROM products.products
    ...
    /// ```no_run
    /// let pool = db.pool();
    /// let products = sqlx::query!("SELECT * FROM products").fetch_all(pool).await?;
    /// ```
