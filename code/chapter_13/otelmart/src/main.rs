...
async fn main() -> Result<()> {
    ...
    // Create an auth-specific router to apply our security audit middleware
    let auth_router = Router::new()
        .route("/register", post(handlers::auth::register))
        .route("/login", post(handlers::auth::login))
        .route("/logout", post(handlers::auth::logout))
        .layer(axum::middleware::from_fn(middleware::security_audit_middleware));
    ...
    let app = Router::new()
        ...
        .nest("/api", api_router)
        // 1. Enforce a maximum of 1000 concurrent requests; LoadShedLayer
        //    immediately returns 503 when the limit is reached (instead of queuing).
        //    HandleErrorLayer converts the BoxError from LoadShedLayer into a Response.
        .layer(
            ServiceBuilder::new()
        ...
        // 2. Add our custom middleware to catch load shedding
        .layer(axum::middleware::from_fn(middleware::track_load_shedding))
