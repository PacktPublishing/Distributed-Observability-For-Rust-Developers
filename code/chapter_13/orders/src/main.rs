...
impl AppState {
    ...
    /// Get the async-phase starvation warning threshold in milliseconds
    pub fn starvation_warn_ms(&self) -> u64 {
        self.starvation_warn_ms
    }
}
...
async fn main() -> Result<()> {
    ...
    let _telemetry = telemetry::init_telemetry("orders");

    // Load configuration from config.toml
    let config = Config::load()?;

    // Initialize async diagnostics instruments and admission control
    ...
    let meter = opentelemetry::global::meter("orders-service");
    ...
    let state = AppState {
        ...
        starvation_warn_ms: config.async_runtime.starvation_warn_ms,
    };

    // Build automatic HTTP RED metrics layer
    let metrics = HttpMetricsLayerBuilder::new().build();

    let backpressure_state = std::sync::Arc::new(middleware::BackpressureState::new(config.async_runtime.max_inflight_checkouts));
    ...
    let app = Router::new()
        ...
        // Admin / Incident simulation endpoints (Chapter 10)
        .route(
            "/admin/report/bug",
            post(handlers::trigger_report_bug),
        )
        .route(
            "/admin/report/fix",
            post(handlers::trigger_report_fixed),
        )
        // Apply our custom bounds via middleware
        .layer(axum::middleware::from_fn_with_state(backpressure_state, middleware::backpressure_layer))
    ...
    info!(
        "    POST   /orders/{{uuid}}/shipment          - Create shipment (sets status='shipped')"
    );
