//! Telemetry initialization for the Orders service.
//!
...
pub fn init_telemetry(service_name: &str) -> TelemetryGuard {
    ...
    // Initialize Tokio Console layer if explicitly enabled via environment variable
    let console_enabled =
        std::env::var("TOKIO_CONSOLE_ENABLED").unwrap_or_else(|_| "0".into()) == "1";

    // Include opentelemetry=off to prevent bridge feedback loop.
    // When Tokio Console is enabled, force Tokio runtime trace directives so
    // task/resource events are not filtered out by a restrictive RUST_LOG value.
    let mut env_filter = EnvFilter::try_from_default_env()
        .unwrap_or_else(|_| EnvFilter::new("info,opentelemetry=off"));

    if console_enabled {
        if let Ok(tokio_trace) = "tokio=trace".parse() {
            env_filter = env_filter.add_directive(tokio_trace);
        }
        if let Ok(runtime_trace) = "runtime=trace".parse() {
            env_filter = env_filter.add_directive(runtime_trace);
        }
    }

    // Console output: JSON in production, compact in development
    ...
    let fmt_layer = if is_production {
        ...
        // When Tokio Console is enabled, include source file and line number in
        // local log output so warning/error events can be traced back to code.
        if console_enabled {
            tracing_subscriber::fmt::layer()
                .with_target(true)
                .with_thread_ids(false)
    ...
    let console_layer = if console_enabled {
        Some(
            console_subscriber::ConsoleLayer::builder()
                .with_default_env() // <-- Adds support for standard env vars like `TOKIO_CONSOLE_FILTER` and `TOKIO_CONSOLE_RETENTION`
...
/// Initialize the logger provider with OTLP HTTP exporter for Loki.
...
pub fn register_pool_metrics(
    ...
    meter
        ...
        .with_callback(move |observer| {
            let active = pool_clone
                .size()
                .saturating_sub(pool_clone.num_idle() as u32);
