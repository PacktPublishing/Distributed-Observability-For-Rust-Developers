...
async fn main() -> Result<()> {
    ...
    // Chapter 8: register the inventory.stock.level gauge with a `category`
    // attribute so the BI dashboard can show stock visibility per category.
    telemetry::register_stock_level_gauge(&meter, db.pool().clone());
