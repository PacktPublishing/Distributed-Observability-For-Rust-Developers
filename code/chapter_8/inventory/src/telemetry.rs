...
/// Per-category stock snapshot used by the `inventory.stock.level` gauge.
///
/// The OpenTelemetry async-gauge callback runs synchronously and cannot
/// itself execute a database query, so a background task periodically
/// refreshes this map and the callback simply observes from it.
type StockByCategory = std::collections::HashMap<String, i64>;

/// Register the `inventory.stock.level` observable gauge.
///
/// Chapter 8 introduces this gauge to support per-category stock visibility
/// in Prometheus and the Business Intelligence dashboard. Each observation
/// carries a `category` attribute set to the product's root category name
/// (e.g., "Electronics", "Clothing"), keeping cardinality low (typically
/// fewer than 20 distinct values).
///
/// # Arguments
/// * `meter` - The meter to register the gauge on
/// * `pool` - The sqlx connection pool used for periodic refresh queries
pub fn register_stock_level_gauge(
    meter: &opentelemetry::metrics::Meter,
    pool: sqlx::Pool<sqlx::Postgres>,
) {
    ...
    // Shared cache that the callback reads from. Updated on a background
    // task every 15 seconds, matching the Prometheus scrape interval.
    let cache: Arc<RwLock<StockByCategory>> = Arc::new(RwLock::new(StockByCategory::new()));

    // Spawn the refresh task. Using sqlx's async API requires being inside
    // a Tokio runtime, which is the case because this is called from main().
    ...
    tokio::spawn(async move {
        ...
        loop {
            ticker.tick().await;
            // Aggregate available stock by root category. The inventory
            // search_path includes `products`, so the join works without
            // a fully qualified schema reference.
            //
            // Implementation note: in this demo seed, real stock counts
            // live in `products.products.stock_quantity` (the
            // `inventory.product_inventory` table is provisioned but not
            // populated by the chapter-7 data ingestion script). Querying
            // the products table keeps the gauge meaningful out of the
            // box for chapter 8 dashboards.
            let rows = sqlx::query_as::<_, (Option<String>, Option<i64>)>(
                r#"
                SELECT
                    COALESCE(get_root_category_name(p.category_id), 'uncategorized') AS category,
                    SUM(p.stock_quantity)::BIGINT AS total_available
            ...
            match rows {
                Ok(rows) => {
                    let mut snapshot = StockByCategory::new();
                    for (cat, total) in rows {
                        let cat = cat.unwrap_or_else(|| "uncategorized".to_string());
                        snapshot.insert(cat, total.unwrap_or(0));
                    }
                    if let Ok(mut guard) = cache_writer.write() {
                        *guard = snapshot;
                    }
                }
                Err(e) => {
                    tracing::warn!(
                        error.message = %e,
                        "Failed to refresh inventory.stock.level cache"
                    );
                }
            }
        }
    ...
    // The async gauge callback reads from the cache and emits one
    // observation per category, with `category` as the attribute key.
    let cache_reader = Arc::clone(&cache);
    meter
        .i64_observable_gauge("inventory.stock.level")
        .with_description("Current available stock units, broken down by product category")
        .with_unit("units")
        .with_callback(move |observer| {
            if let Ok(snapshot) = cache_reader.read() {
                for (category, qty) in snapshot.iter() {
                    observer.observe(
                        *qty,
                        &[opentelemetry::KeyValue::new("category", category.clone())],
                    );
                }
            }
        })
