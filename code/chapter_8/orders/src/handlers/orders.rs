...
struct ProductDetail {
    ...
    stock: i32,
    is_active: bool,
    // Top-level category (e.g., "Electronics", "Clothing"). Used in
    // chapter 8 for the `order.category` log attribute. Optional because
    // a product may not yet be assigned a category.
    #[serde(default)]
...
/// Compute a single root-category label for an order.
///
/// Used in chapter 8 for the `order.category` log attribute. Returns the
/// shared root category if every line item belongs to the same category,
/// otherwise returns "mixed". This keeps the log field's cardinality
/// bounded to the small set of root categories plus the literal "mixed"
/// and "uncategorized" sentinels.
fn dominant_category(categories: &[String]) -> String {
...
pub async fn create_order(
    ...
    // Track the root category of every line item so we can emit a single
    // `order.category` log attribute on success. Chapter 8 uses this in
    // Loki to aggregate revenue by category.
    let mut item_categories: Vec<String> = Vec::with_capacity(request.items.len());
    ...
    for item in &request.items {
        ...
        // Capture this product's root category for the order-level log field.
    ...
    match result {
        Ok(created_order) => {
            ...
            // Compute the order-level category attribute. If every line item
            // belongs to the same root category, use it directly. If items
            // span multiple root categories, mark the order as "mixed".
            // This keeps the cardinality of the log field bounded.
            let order_category = dominant_category(&item_categories);
