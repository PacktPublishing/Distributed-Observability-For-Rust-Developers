...
async fn spa_fallback_handler(
    ...
    match tokio::fs::read_to_string(&index_path).await {
        ...
        Err(_) => Response::builder()
            ...
            .header("content-type", "text/html; charset=utf-8")
            .body(Body::from("<h1>UI not built</h1><p>Please build the Angular application first.</p>"))
            .unwrap()
            .into_response(),
    }
}
...
async fn main() -> Result<()> {
    ...
    println!("Orders Service: {}:{}", config.services.orders_host, config.services.orders_port);
    println!("Checkout Service: {}:{}", config.services.checkout_host, config.services.checkout_port);
    println!("Static files directory: {}", config.server.static_dir);
    ...
    let state = AppState {
        ...
        checkout_service_url: config.services.checkout_service_url(),
    };
    ...
    let api_router = Router::new()
        ...
        .route("/users/addresses/{id}", post(handlers::users::update_address))
        .route("/users/addresses/{id}", axum::routing::delete(handlers::users::delete_address))
        ...
        .route("/products", axum::routing::any(proxy::products::proxy_products))
        .route("/products/{*path}", axum::routing::any(proxy::products::proxy_products))

        // Proxy all /inventory/* requests to inventory service
        .route("/inventory", axum::routing::any(proxy::inventory::proxy_inventory))
        .route("/inventory/{*path}", axum::routing::any(proxy::inventory::proxy_inventory))

        // Proxy all /orders/* requests to orders service
        .route("/orders", axum::routing::any(proxy::orders::proxy_orders))
        .route("/orders/{*path}", axum::routing::any(proxy::orders::proxy_orders))
    ...
    let app = Router::new()
        ...
        .fallback(move |uri: Uri, req: Request<Body>| {
    ...
    println!("\nOtelMart Service listening on {}", addr);
