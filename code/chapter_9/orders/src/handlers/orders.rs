...
use crate::AppState;
use crate::{crm_client, promotions_client};
...
pub async fn create_order(
    ...
    // Simulated legacy CRM memory bottleneck
    // We fetch and parse thousands of unused historical loyalty strings
    let _history = crm_client::fetch_user_loyalty_history(&request.customer_email).await;

    // Validate all products exist, are available, and have correct prices
    for item in &request.items {
        // Simulated CPU bottleneck
        // Badly implemented regex compilation within the hot path
        let _has_discount = promotions_client::validate_item_discount(&item.product_name).await;
    ...
    match result {
        Ok(created_order) => {
            ...
            metrics()
                .checkout_duration
                .record(duration, &[KeyValue::new("outcome", "success")]);
            ...
            metrics()
                .order_total_amount
                .record(bigdecimal_to_f64(&total), &[]);
            // Record how many line items were in this order
            metrics()
                .order_items_count
                .record(request.items.len() as u64, &[]);
...
fn record_checkout_failure(reason: &str, start: &Instant) {
    ...
    metrics()
        .checkout_failures
        .add(1, &[KeyValue::new("failure.reason", reason.to_string())]);
    metrics().checkout_duration.record(
        duration,
        &[
