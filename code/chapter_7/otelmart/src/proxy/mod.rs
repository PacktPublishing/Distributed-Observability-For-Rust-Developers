...
pub async fn proxy_request(
    ...
    let response = match response {
        ...
        Err(e) => {
            ...
            // Log the downstream service failure with structured fields
            tracing::error!(
                error.r#type = "http_client",
                error.message = %e,
                downstream.service = %upstream_service,
                downstream.url = %target_url,
                downstream.duration_ms = format!("{:.1}", duration * 1000.0),
    ...
    // Log completed downstream call with response status
    tracing::info!(
        downstream.service = %upstream_service,
        downstream.status = status.as_u16(),
        downstream.duration_ms = format!("{:.1}", duration * 1000.0),
