...
#[instrument(
    ...
    fields(
        ...
        async.queue.wait_ms = tracing::field::Empty,
        async.checkout.state = tracing::field::Empty,
    )
)]
pub async fn create_order(
    ...
    for item in &request.items {
        let product_result = validate_product(&state, item.product_uuid).await;
    ...
    for item in &request.items {
        let reserve_result = reserve_stock(&state, item.product_uuid, item.quantity).await;
